...
/**
 * Check if a username is available across users and channels/groups tables.
 * @param {string} username - The username to check
 * @param {number|null} excludeUserId - Exclude a specific user (e.g. the channel owner updating their own channel)
 * @returns {boolean} true if available, false if taken
 */
function isUsernameAvailable(username, excludeUserId) {
  if (!username || !/^[a-zA-Z0-9_]{4,30}$/.test(username)) return false;

  // Check users table
  const userQuery = excludeUserId
    ? 'SELECT 1 FROM users WHERE username = ? AND id != ?'
    : 'SELECT 1 FROM users WHERE username = ?';
  const userParams = excludeUserId ? [username, excludeUserId] : [username];
  if (db.prepare(userQuery).get(...userParams)) return false;

  // Check channels table (username column)
  if (db.prepare('SELECT 1 FROM channels WHERE username = ?').get(username)) return false;

  // Check groups table (username column)
  if (db.prepare('SELECT 1 FROM groups WHERE username = ?').get(username)) return false;
