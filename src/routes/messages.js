...
router.post('/upload', (req, res) => {
  mediaUpload.single('file')(req, res, (err) => {
    ...
    const attachmentType = req.file.mimetype.startsWith('video/') ? 'video' : req.file.mimetype.startsWith('audio/') ? 'audio' : req.file.mimetype.includes('gif') ? 'gif' : req.file.mimetype.startsWith('image/') ? 'photo' : 'file';
    let mediaId = null;
    try {
      const info = db.prepare(`INSERT INTO media_files (owner_id, file_path, original_name, mime_type, size_bytes, encrypted, encryption_meta_json) VALUES (?, ?, ?, ?, ?, ?, ?)`)
        .run(req.userId, req.file.filename, req.file.originalname, req.file.mimetype, req.file.size, req.body?.encrypted ? 1 : 0, req.body?.encryptionMeta || null);
...
router.get('/conversations', (req, res) => {
  ...
  const conversations = rows.map((row) => {
    ...
    const peer = db.prepare('SELECT id, username, nickname, box_public_key, avatar_path, is_premium, premium_emoji FROM users WHERE id = ?').get(peerId);
    ...
    return {
      ...
      peer: {
        ...
        avatarUrl: peer.avatar_path ? `/avatars/${peer.avatar_path}` : null,
        isPremium: Boolean(peer.is_premium),
        premiumEmoji: peer.premium_emoji,
      },
...
router.get('/with/:username', (req, res) => {
  ...
  res.json({
    ...
    peer: { id: peer.id, username: peer.username, nickname: peer.nickname, boxPublicKey: peer.box_public_key, avatarUrl: peer.avatar_path ? `/avatars/${peer.avatar_path}` : null, isPremium: Boolean(peer.is_premium), premiumEmoji: peer.premium_emoji },
